...
# data_for_plot_uk <- ecdc_data %>%
#   filter(country == "France")
# 
# ggplot(data_for_plot_uk, mapping = aes(x = date, y = cumulative_cases)) +
#   geom_point() +
#   theme_minimal() +
#   ggtitle("Number of cases in the United Kingdom") +
#   xlab("Date") +
#   ylab("Number of cases") +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
#   theme(plot.title = element_text(hjust = 0.5)) +
#   labs(caption = "Source : ECDC (https://www.ecdc.europa.eu/en/publications-data/download-todays-data-geographic-distribution-covid-19-cases-worldwide) - Chart by Philippe Lemoine (@phl43) for Quillette") +
#   ggsave("Number of cases in the United Kingdom.png", width = 12, height = 6)
# 
# ggplot(data_for_plot_uk, mapping = aes(x = date, y = cumulative_deaths)) +
#   geom_point() +
#   theme_minimal() +
#   ggtitle("Number of deaths in the United Kingdom") +
#   xlab("Date") +
#   ylab("Number of deaths") +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
#   theme(plot.title = element_text(hjust = 0.5)) +
#   labs(caption = "Source : ECDC (https://www.ecdc.europa.eu/en/publications-data/download-todays-data-geographic-distribution-covid-19-cases-worldwide) - Chart by Philippe Lemoine (@phl43) for Quillette") +
#   ggsave("Number of deaths in the United Kingdom.png", width = 12, height = 6)
# 
# 
# 
# 
...
# 
# 
# training_start_cases <- ymd("2020-03-30")
# training_end_cases <- ymd("2020-04-15")
# training_start_deaths <- ymd("2020-03-30")
# training_end_deaths <- ymd("2020-04-15")
# plot_length <- 45
# 
# data_for_linear_fit_uk <- ecdc_data %>%
#   filter(country == "United Kingdom" & date >= training_start_cases) %>%
#   mutate(days_since_start = as.integer(date - training_start_cases))
# 
# fit_cases_uk <- lm(
#   cumulative_cases ~ days_since_start,
#   filter(data_for_linear_fit_uk, date >= training_start_cases & date <= training_end_cases)
# )
# 
# fit_deaths_uk <- lm(
#   cumulative_deaths ~ days_since_start,
#   filter(data_for_linear_fit_uk, date >= training_start_deaths & date <= training_end_deaths)
# )
# 
# data_for_linear_fit_uk$predicted_cases <- round(predict(fit_cases_uk, data_for_linear_fit_uk))
# data_for_linear_fit_uk$predicted_deaths <- round(predict(fit_deaths_uk, data_for_linear_fit_uk))
# 
# data_for_plot_uk <- filter(data_for_linear_fit_uk, days_since_start >= 0 & days_since_start <= plot_length)
# 
# ggplot(data_for_plot_uk, mapping = aes(x = date, y = cumulative_cases)) +
#   geom_point() +
#   geom_line(aes(y = predicted_cases), size = 1, color = "steelblue") +
#   annotate(
#     geom = "text",
#     label = paste0("R^2 == ", as.character(round(summary(fit_cases_uk)$r.squared, 2))),
#     x = min(data_for_plot_uk$date),
#     y = 0.985 * max(data_for_plot_uk$predicted_cases),
#     hjust = 0,
#     vjust = 1,
#     parse = TRUE
#   ) +
#   theme_minimal() +
#   ggtitle(paste0("Number of cases in the United Kingdom and prediction of a linear model (training set = data from ", training_start_cases, " to ", training_end_cases, ")")) +
#   xlab("Date") +
#   ylab("Number of cases") +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
#   theme(plot.title = element_text(hjust = 0.5)) +
...
#   annotate(
#     geom = "text",
#     label = paste0("R^2 == ", as.character(round(summary(fit_deaths_uk)$r.squared, 2))),
#     x = min(data_for_plot_uk$date),
#     y = 0.985 * max(data_for_plot_uk$predicted_deaths),
#     hjust = 0,
#     vjust = 1,
#     parse = TRUE
#   ) +
#   theme_minimal() +
#   ggtitle(paste0("Number of deaths in the United Kingdom and prediction of a quadratic model (training set = data from ", training_start_cases, " to ", training_end_cases, ")")) +
#   xlab("Date") +
#   ylab("Number of deaths") +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
#   theme(plot.title = element_text(hjust = 0.5)) +
...
# 
# 
# ggplot(filter(ecdc_data, country == "United Kingdom"), mapping = aes(x = date, y = cumulative_cases)) +
#   geom_point() +
#   theme_minimal() +
#   ggtitle(paste0("Number of cases in the United Kingdom and prediction of a linear model (training set = data from ", training_start_cases, " to ", training_end_cases, ")")) +
#   xlab("Date") +
...
# 
# 
# ggplot(data_cfr_plot, aes(x = country, y = CFR)) +
#   geom_bar(stat = "identity", color = "black", fill = "steelblue") +
#   geom_text(aes(label = paste0(round(CFR * 100, 1), "%"), vjust = -0.5),
#             color = "black",
#             size = 3) +
#   theme_minimal() +
#   ggtitle("Case fatality rate of COVID-19 for a selection of countries") +
#   xlab("Country") +
...
#   theme(plot.title = element_text(hjust = 0.5)) +
#   labs(caption = "Source : ECDC (https://www.ecdc.europa.eu/en/publications-data/download-todays-data-geographic-distribution-covid-19-cases-worldwide) - Chart by Philippe Lemoine (@phl43) for Quillette") +
#   ggsave("Case fatality rate of COVID-19 for a selection of countries.png", width = 12, height = 6)
# 
# countries <- c(
#   "China",
#   "Italy",
...
# )
# 
# data_death_plot <- filter(ecdc_data, country %in% countries) %>%
#   filter(date == max(date))
# 
# ggplot(data_death_plot, aes(x = country, y = death_rate)) +
#   geom_bar(stat = "identity", color = "black", fill = "steelblue") +
#   geom_text(aes(label = round(death_rate, 1), vjust = -0.5),
#             color = "black",
#             size = 3) +
#   theme_minimal() +
#   ggtitle("Number of deaths per 100,000 from COVID-19 for a selection of countries") +
#   xlab("Country") +
...
#   theme(plot.title = element_text(hjust = 0.5)) +
#   labs(caption = "Source : ECDC (https://www.ecdc.europa.eu/en/publications-data/download-todays-data-geographic-distribution-covid-19-cases-worldwide) - Chart by Philippe Lemoine (@phl43) for Quillette") +
#   ggsave("Number of deaths per 100,000 from COVID-19 by country.png", width = 12, height = 6)
# 
# 
# 
# country_for_quadratic_fit <- "China"
# training_start_cases <- ymd("2020-01-20")
# training_end_cases <- ymd("2020-02-04")
# training_start_deaths <- ymd("2020-01-20")
# training_end_deaths <- ymd("2020-02-04")
# plot_length <- 45
# 
# # country_for_quadratic_fit <- "South Korea"
# # training_start_cases <- ymd("2020-02-15")
# # training_end_cases <- ymd("2020-02-29")
# # training_start_deaths <- ymd("2020-02-15")
# # training_end_deaths <- ymd("2020-02-29")
# # plot_length <- 45
# 
# # country_for_quadratic_fit <- "United Kingdom"
# # training_start_cases <- ymd("2020-03-30")
# # training_end_cases <- ymd("2020-04-15")
# # training_start_deaths <- ymd("2020-03-30")
# # training_end_deaths <- ymd("2020-04-15")
# # plot_length <- 45
# 
# data_for_quadratic_fit <- ecdc_data %>%
#   filter(country == country_for_quadratic_fit & date >= training_start_cases) %>%
#   mutate(days_since_start = as.integer(date - training_start_cases))
# 
# fit_cases <- lm(
#   cumulative_cases ~ days_since_start + I(days_since_start^2),
#   filter(data_for_quadratic_fit, date >= training_start_cases & date <= training_end_cases)
#   )
# 
# # fit_cases <- lm(
# #   cumulative_cases ~ days_since_start,
...
# fit_deaths <- lm(
#   cumulative_deaths ~ days_since_start + I(days_since_start^2),
#   filter(data_for_quadratic_fit, date >= training_start_deaths & date <= training_end_deaths)
# )
# 
# # fit_deaths <- lm(
# #   cumulative_deaths ~ days_since_start,
...
# # )
# 
# data_for_quadratic_fit$predicted_cases <- round(predict(fit_cases, data_for_quadratic_fit))
# data_for_quadratic_fit$predicted_deaths <- round(predict(fit_deaths, data_for_quadratic_fit))
# 
# data_for_plot <- filter(data_for_quadratic_fit, days_since_start >= 0 & days_since_start <= plot_length)
# 
# ggplot(data_for_plot, mapping = aes(x = days_since_start, y = cumulative_cases)) +
#   geom_point() +
#   geom_line(aes(y = predicted_cases), size = 1, color = "steelblue") +
#   annotate(
#     geom = "text",
#     label = paste0("R^2 = ", as.character(round(summary(fit_cases)$r.squared, 2))),
#     x = min(data_for_plot$days_since_start),
#     y = 0.985 * max(data_for_plot$cumulative_cases),
#     hjust = 0,
#     vjust = 1
#   ) +
#   theme_minimal() +
#   ggtitle(paste0("Number of cases in ", country_for_quadratic_fit, " and prediction of a quadratic model (training set = data from ", training_start_cases, " to ", training_end_cases, ")")) +
#   xlab(paste0("Number of days since ", training_start_cases)) +
#   ylab("Number of cases") +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
#   theme(plot.title = element_text(hjust = 0.5)) +
#   labs(caption = "Source : ECDC (https://www.ecdc.europa.eu/en/publications-data/download-todays-data-geographic-distribution-covid-19-cases-worldwide) - Chart by Philippe Lemoine (@phl43) for Quillette")
# 
# ggplot(data_for_plot, mapping = aes(x = days_since_start, y = cumulative_deaths)) +
#   geom_point() +
#   geom_line(aes(y = predicted_deaths), size = 1, color = "steelblue") +
#   annotate(
#     geom = "text",
#     label = paste0("R^2 = ", as.character(round(summary(fit_deaths)$r.squared, 2))),
#     x = min(data_for_plot$days_since_start),
#     y = 0.985 * max(data_for_plot$cumulative_deaths),
#     hjust = 0,
#     vjust = 1
#   ) +
#   theme_minimal() +
#   ggtitle(paste0("Number of deaths in ", country_for_quadratic_fit, " and prediction of a quadratic model (training set = data from ", training_start_cases, " to ", training_end_cases, ")")) +
#   xlab(paste0("Number of days since ", training_start_cases)) +
#   ylab("Number of deaths") +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
#   theme(plot.title = element_text(hjust = 0.5)) +
...
#   geom_point(aes(y = actual_proportion)) +
#   theme_minimal() +
#   ggtitle("Benford's law analysis for the daily number of new cases") +
#   xlab("Digit") +
#   ylab("Proportion") +
#   scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
#   theme(plot.title = element_text(hjust = 0.5)) +
...
#   geom_point(aes(y = actual_proportion)) +
#   theme_minimal() +
#   ggtitle("Benford's law analysis for the daily number of new deaths") +
#   xlab("Digit") +
#   ylab("Proportion") +
#   scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
#   theme(plot.title = element_text(hjust = 0.5)) +
...
# x <- 10
# 
# (x^4 + 15*x^2 - 6*x)/((x^2 + 5)^2)
# (-x^3 + x^2 - 6*x)/(x^2 + 5)
# 
# # ecdc_latest_date <- max(ecdc_data$date)
# # 
# # ecdc_data_latest <- ecdc_data %>%
# #   filter(date == ecdc_latest_date)